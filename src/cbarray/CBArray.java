...
import static cbarray.CharSize.*; // The size of this font
...
//      codes[crow++].setText(new String(new Integer(code).toHexString(code)));
      ...
      {
          System.out.println("itemStateChanged(): only 16 & 32 bits supported; rows="+rows);
      }
    } // endforeach column
//System.out.println("");
...
//System.out.print("n="+i+":"+sHexCode+";");
      // sHexCode now has a 32 bit wide value; if this is a 32 row character,
      // we can use it as it is; otherwise, we have to truncate to 16 bits
//      if(rows==32)
//      {
          s += sHexCode;
//      }
//      else
//      {
//          String sIntHexCode = new String("0x" + sHexCode.substring(6,8));
//          s += sIntHexCode; // just take the last 4 hex digits
//      }
      if(i < cols-1) s += ',';
    }
System.out.println();
...
//    int ctmp;
    ...
    int crow=0;
    long mask=0x1;
    mask <<= (rows-1); // adjust mask for # columns
    for(col=0; col<cols; col++)
    {
...
//System.out.println("disprec; code="+code);
      ...
      {
//        ctmp = code & 0x1;
        bits[row][col].setSelected((code & mask) != 0);
...
if(cols == 16)
{
  s = String.format("0x%4s", new Integer(i).toHexString(i)).replace(' ','0');
}
else if(cols == 32)
{
  s = String.format("0x%8s", new Integer(i).toHexString(i)).replace(' ','0');
}
*/
...
System.out.println(ex);
  ...
  {
      // 2015Dec26 Don't trash main file - it is used for display and editing
      List<LineBuf> as8BitFileContents = new ArrayList<LineBuf>(asFileContents);

    StringBuffer sbFullPathTo8BitFile = new StringBuffer(sFullPathToFile);
    int j = sbFullPathTo8BitFile.indexOf(".h");
    if(j > -1)
    {
        sbFullPathTo8BitFile.delete(j, sbFullPathTo8BitFile.length()-1);
    }
    sbFullPathTo8BitFile.append("8Bit.h");
System.out.println("saveFileAs8Bit() to filename:"+sbFullPathTo8BitFile.toString());
// Assuming as8BitFileContents has already been parsed and cleaned up above....
    int p;
    LineBuf buf = new LineBuf();
    StringBuilder sbOutbuf;
    // Convert from 16 or 32 bit words to 8 bit values
    for(int i=0; i<as8BitFileContents.size(); i++)
    {
      buf = as8BitFileContents.get(i);
...
//String[] hexnumstrings = buf.sLine.split(",");
      ...
      if(p > -1) // if a code is present...
      {
        for(int col=0; col<cols; col++)
        {
          // find first "0x" of this output 16 or 32 bit word
          p = buf.sLine.indexOf("0x", p);
System.out.println("sfa8b#2 p="+p);
...
System.out.println("foreachbyte-"+byt+":"+sbOutbuf.toString());
            ...
            if(byt<(rows/8)-1 || col<cols-1) sbOutbuf.append(","); // if not last row && last col, append a comma
            p += 2;
          }
//        if(col < cols) sbOutbuf.append(",");
        } // endfor each column
        if(i<as8BitFileContents.size()-1) // if not last line
        {
          sbOutbuf.append("},");
    ...
    try
    {
System.out.println("Saving 8 bit only to:\""+sbFullPathTo8BitFile.toString()+"\"");
      PrintWriter writer = new PrintWriter(sbFullPathTo8BitFile.toString());
// 2015Dec26 Add header and enclosing parenthesis so file will be ready to use...
writer.println("// 32x32 font");
writer.println("const char myFont32x32[][128] PROGMEM = {");
      for(int i=0; i<as8BitFileContents.size(); i++)
      {
        writer.println(as8BitFileContents.get(i).sLine);
      }
writer.println("};");
